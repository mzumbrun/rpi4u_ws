...
void setup() {
  ...
  pinMode(m1_encoder_interrupt, INPUT_PULLUP);
  pinMode(m2_encoder_interrupt, INPUT_PULLUP);
  // Set Callback for Wheel Encoders Pulse
  //attachInterrupt(digitalPinToInterrupt(m2_encoder_interrupt), rightEncoderCallback, RISING);
  //attachInterrupt(digitalPinToInterrupt(m1_encoder_interrupt), leftEncoderCallback, RISING);
}
...
void loop() {
 ...
 //     is_cmd_complete = false;
    ...
    // Positive direction
      ...
      } else if (is_m2_wheel_cmd && !is_m2_wheel_forward) {
  ...
  if (current_millis - last_millis >= interval) {
    ...
    // m2_wheel_meas_vel = m2_encoder_counter * (60.0 / (35.)) * 0.10472;  // rad/s
    // m1_wheel_meas_vel = m1_encoder_counter * (60.0 / (35.)) * 0.10472;
    // skip encoder
    ...
    if (is_m2_wheel_cmd && !is_m2_wheel_forward) {
      m2_servo.writeMicroseconds(map(m2_wheel_cmd, 0, 200, 1500, 600));
      m2_wheel_sign = "n";
    }
  }
}
